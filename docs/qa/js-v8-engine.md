...
## AOT vs JIT vs 인터프리터 비교

| 방식 | 특징 | 예시 |
|------|------|------|
| **AOT (Ahead-of-Time)** | 실행 전 전체를 기계어로 변환 | C, Go, Rust |
| **순수 인터프리터** | 줄 단위 실행, 빠른 시작 but 느린 반복 | 초기 JS |
| **JIT** | 처음엔 인터프리터, Hot code만 컴파일 | 현대 JS, Java JVM |

JIT는 **초기 실행 속도 + 반복 실행 성능**을 동시에 챙기는 전략입니다.
